...
def list_watchlists(
    ...
    return service.get_watchlists_grouped(user.id)
...
def create_category(
    ...
    existing_categories = service.get_user_categories(user.id)
...
def get_watchlist(
    ...
    """
    service = WatchlistService(db)
    watchlist = service.get_watchlist(watchlist_id, user.id)
...
def delete_watchlist(
    ...
    """
    service = WatchlistService(db)
    success = service.delete_watchlist(watchlist_id, user.id)
...
def list_symbols(
    ...
    symbols = service.get_watchlist_symbols(watchlist_id, user.id)
...
def clone_watchlist(
    ...
    original_watchlist = service.get_watchlist(watchlist_id, user.id)
    ...
    existing_watchlists = service.get_user_watchlists(user.id)
    ...
    if clone_data.category_id is not None:
        cloned_watchlist.category_id = clone_data.category_id
    if clone_data.description is not None:
        cloned_watchlist.description = clone_data.description

    db.commit()
    db.refresh(cloned_watchlist)

    # Load symbols for response
    symbols = service.get_watchlist_symbols(cloned_watchlist.id, user.id)
